...
async function makePp(buf) {
  ...
  const JimpModule = (await import("jimp")).default || (await import("jimp"));
  const Jimp = JimpModule;
  const img = await Jimp.read(buf);
  // Make square crop from center
  const size = Math.min(img.getWidth(), img.getHeight());
  const x = Math.floor((img.getWidth() - size) / 2);
  const y = Math.floor((img.getHeight() - size) / 2);
  const crop = img.clone().crop(x, y, size, size).resize(320, 320); // 320x320 profile pic
  const imgBuf = await crop.quality(90).getBufferAsync(Jimp.MIME_JPEG);
  // small preview: smaller sized
  const prev = await crop.clone().resize(64, 64).getBufferAsync(Jimp.MIME_JPEG);
  return { img: imgBuf, prev };
}
...
const serialize = async (msg, conn) => {
  ...
  async function downloadMediaFromMsg(wamessage, mediaType = null) {
    ...
    const downloadFn = baileys.downloadMediaMessage || baileys.downloadContentFromMessage || null;
    ...
    try {
      if (downloadFn === baileys.downloadMediaMessage) {
        ...
        const stream = await downloadFn(content, targetType.replace("Message", ""));
        const chunks = [];
  ...
  msgObj.download = async () => {
    try {
      // Use the full raw message (WAMessage)
      return await downloadMediaFromMsg(msg.raw, null);
    } catch (err) {
